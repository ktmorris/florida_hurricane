...
Table \ref{tab:full-dind} formalizes Figure \ref{fig:full-to} into a differences-in-differences regression specification. I employ an ordinary least squares specification.^[Although the dependent variable here is binary --- it takes the value 0 if a voter does not participate, and 1 if she does --- the coefficients produced by logistic regressions in the difference-in-differences context are largely uninterpretable. I thus use a linear specification here. When the models are estimated using a logistic specification, the treatment effect is virtually identical.] The dependent variable takes the value 1 if a voter cast a ballot in a given year, and 0 if she did not. Model 1 includes only three variables in addition to the constant. *Treated* measures the gap between treated and control voters in the 2010 -- 2016 period. *2018* measures the increase in turnout observed among control voters in 2018, while *Treated × 2018* measures whether turnout in 2018 departed further or less from the baseline for the treated voters than the control voters. Model 2 includes the same variables, but also includes the characteristics on which the voters were matched. Model 3, finally, also includes measures for congressional district competitiveness. Because this variable is "downstream" of treatment --- that is to say, the effect of the hurricane could have impacted the competitiveness of certain races --- it is not included in the first two models. It should be noted that each of the treated voters lived in uncontested congressional districts. Robust standard errors are clustered at the level of the match [@Abadie2019].

\begin{singlespace}
\input{"../temp/dind_full.tex"}
\end{singlespace}

The coefficient on *Treated × 2018* in Table \ref{tab:full-dind} indicates that Hurricane Michael, net of any county- and state-level administrative responses, reduced turnout in 2018 among the treated voters. The coefficients in Table \ref{tab:full-dind}, however, are difficult to interpret. To understand the magnitude of the depressive effect, we calculate what each voter's predicted probability of turning out in 2018 would have been absent the treatment effect. We do so by using each of the coefficients in the regression output except for *Treated × 2018*. These predicted probabilities indicate that turnout among treated voters in 2018 should have been between 67.4 and 67.7 percent. Given that the observed turnout for this group was 58.5 percent, this implies that Hurricane Michael reduced turnout by roughly 9 percentage points. Multiplied across the nearly 200 thousand registered voters in the treated counties indicates that some 18 thousand ballots went uncast due to the hurricane --- a major effect in a year when a statewide senate race was decided by 10,033 votes.
...
In each model in Table \ref{tab:trip-diff}, *Panhandle × 2018* is statistically nonsignificant. This indicates that the primary control voters did not turn out at lower rates than their secondary control voters from around the state. This is surprising --- it indicates that, despite their proximity to counties that were badly damaged by Hurricane Michael, the hurricane had *no effect* on their 2018 turnout.

The coefficient on *Treated × 2018*, however, is statistically significant and negative. This indicates that treated voters in the counties covered by the executive order turned out at lower rates than voters in bordering untreated counties. The depressed turnout for these voters, therefore, can be explained entirely by *administrative* effects, and not by direct any individual effects of the storm on propensity to vote.

As before, the direct interpretation of the coefficient on *Treated × 2018* is hampered by the nature of the logistic model. Once again, we use the estimated models to approximate what turnout for treated voters would have been in 2018 absent the administrative effects. Although observed turnout among treated voters in 2018 was just 65.3 percent, the triple-differences models each estimate that it would have been 68.2 percent without the administrative effect of the storm --- even after accounting for the storm's individual-level effects. We estimate, therefore, that the administrative effect of the storm was to reduce turnout by roughly 2.9 percentage points. This effect is significant at the 99 percent level.
