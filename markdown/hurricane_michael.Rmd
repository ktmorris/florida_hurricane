...
Because we are interested in the overall administrative effect of the hurricane on turnout, we must weight each of our treated voters (and their subsequent matches). Otherwise, our estimates will be skewed based on the number of voters in a given county that happened to live within two miles of the border. For instance, Bay County has more than 100 thousand registered voters, but just 975 voters live within two miles of an untreated county. Gadsden County, on the other hand, has fewer than 27 thousand registered voters overall, but 1,782 of them lived within two miles of a border. Without reweighting the treated observations, Gadsden County's administrative effects would count for more than double the administrative effects of Bay --- despite the former county having just over 25 percent of the population of the latter. We therefore weight each treated voter (and her subsequent matches) by their county's total population divided by the share that lives in the buffer zone. Bay County treated voters, therefore, will be receive a weight of 105,575 / 975, or `r round(105575 / 975, digits = 1)`, while the weights for Gadsden County are just `r round(26600 / 1782, digits = 1)`.
...
Figure \ref{fig:second-parallel} demonstrates that the parallel trends assumption generally holds: turnout among the secondary controls tracks neatly with the turnout of the Panhandle voters prior to 2018. To the extent that the gap narrows in 2016, this renders our estimates conservative; any depressive effect will be hard to identify, given that the baseline gap was shrinking.

Figure \ref{fig:second-parallel} demonstrates, surprisingly, that turnout among Panhandle voters was moderately higher in 2018 than might have been expected based on historical turnout. This is a reflection of Figure \ref{fig:ll-to} above, which indicated that turnout increased for primary control voters. Of course, Figure \ref{fig:second-parallel} does not allow us to decompose turnout effects for the treated and primary control voters.
...
where individual *i*'s turnout in year *t* is a function of the year and their location. In the equation, *b~1~Pandhandle~i~* measures the historical difference between voters in the panhandle (both treated and matched, untreated individuals) and the rest of the state. *b~2~2018~it~* measures the statewide change in turnout in 2018 from the baseline, while *b~3~Panhandle~i~ × 2018~it~* tests whether turnout changed differently in 2018 in the panhandle than it did elsewhere. *b~3~Panhandle~i~ × 2018~it~*, therefore, is our estimation of the individual-level, or weather related, effect of the hurricane. *b~4~Treated~i~* measures the historical difference between treated and control observations in the buffer, and *b~5~Treated~i~ × 2018~it~* tests whether the change in turnout in 2018 was different for voters living in the treated counties than for their matched controls in the panhandle. We also test whether the secondary control voters for the treatment group had higher or lower turnout than the other set of secondary control voters using *b~6~SecondaryControlGroup1~i~* term.
...
Each model in Table \ref{tab:trip-diff} indicates that turnout in 2018 was substantially higher for the primary control voters; *Panhandle × 2018* estimates that this effect was approximately 9.2 percentage points. Because these voters lived in counties that did not face the same administrative problems as the treated voters, we assume that this captures the individual-level effect of Hurricane Michael. Surprisingly, proximal contact with the storm (though not direct --- these voters lived outside of the hardest-hit counties) appears to have had a mobilizing effect on turnout.

The coefficients on *Treated × 2018* indicate that the turnout for the treated voters who lived just over the county line from the primary control voters was, however, depressed. Because the treated and primary control voters faced the same weather --- and therefore individual effects --- of the storm, this loss in turnout can be attributed to the administrative problems faced by the treated counties. Each model estimates that this effect was -9.4 percentage points. Taken together, *Panhandle × 2018* and *Treated × 2018* indicate that turnout for the treated voters was depressed by 0.2 percentage points in 2018, but that there were positive individual-level effects and negative administrative ones. This overall estimate is lower than that estimated in Table \ref{tab:full-dind}. This not surprising; the treated voters in the triple-differences model live on the outer edges of the treated counties, and therefore likely faced smaller individual-level effects from the storm.
