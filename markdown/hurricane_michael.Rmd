...
p[["data"]] <- p[["data"]] %>% 
  mutate(treated = ifelse(treated == "Treated Voters", "Covered by Executive Order", "Control Voters"))
p[["data"]]$treated <- factor(p[["data"]]$treated, levels = c("Covered by Executive Order", "Control Voters"))
p
```
...
Figure \ref{fig:trip-diff-coef} makes a number of things clear. First, neither model estimates a statistically significant treatment effect for the treated counties *as a whole*, although each individual county's treatment effect is significantly different than 0. This provides further corroboration for the notion that what mattered in the Panhandle in 2018 was how each individual county exercised its latitude under the Executive Order, *not* the Executive Order as a single, monolithic treatment.
